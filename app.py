...
# log_handler.addFilter(LoggingFilter())  # Add APM logging filter here
...
#             logging.info(f"Created uploads folder: {self.upload_folder}")
...
#                 with capture_span('db.insert_metadata', 'db'):
...
#                     logging.info(f"File metadata stored in the database for: {file.filename}")

#             return {"success": True, "details": [file.filename for file in files]}

#         except Exception as e:
#             logging.error(f"Error during file upload or metadata storage: {str(e)}")
...
#         try:
#             while True:
#                 logging.info("Waiting for message from WebSocket client...")
#                 with capture_span('websocket.receive_text', 'websocket'):
#                     data = await websocket.receive_text()
#                     logging.info(f"Received message from client: {data}")

#                 with capture_span('websocket.send_text', 'websocket'):
#                     answer = self.chat_manager.get_response(data)
#                     logging.info(f"Generated response: {answer}")
#                     await websocket.send_text(answer)
#                     logging.info(f"Sent message to client: {answer}")
